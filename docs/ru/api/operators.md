...
concurrency(saveMutation, { strategy: 'QUEUE' }); // записи идут строго по очереди
```

`QUEUE` сериализует прогоны: следующий стартует только после завершения предыдущего,
ошибки цепочку не рвут, а `cancel`/`reset` смывают ожидающих (они абортятся как
`'cancelled'`). В сочетании с ключом полосы сериализация действует внутри полосы.
