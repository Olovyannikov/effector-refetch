...
Обе библиотеки делают SSR по-эффекторовски — `fork()` на запрос, `allSettled`,
`serialize(scope)` для состояния сторов (`$data` / `$status` восстанавливаются без мигания
загрузки). Разница — в **слое кэша**:

- **farfetched** резолвит адаптер через внутренний стор `__.$instance` («to support Fork
  API»), так что подмена инстанса адаптера на форк технически возможна — но по-адаптерно и
  вручную; дефолтный `inMemoryCache` держит записи в замыкании, общем для всех scope, а
  контракт адаптера (`get` / `set` / `purge` / `unset`) не умеет перечислять записи — встроенного
  переноса кэша сервер→клиент нет. Ключи неймспейсятся по sid запроса (для кэширования
  обязателен effector babel/SWC-плагин).
- **effector-refetch** — один переключатель на всё приложение: `fork({ values: [[$queryCache,
inMemoryCache()]] })` изолирует кэш каждого query в этом scope. Записи неймспейсятся по
  query (`name` ?? sid эффекта ?? счётчик, плагин не обязателен), `dehydrate(cache)` /
  `hydrate(cache, snapshot)` переносят записи ровно одного запроса с сохранением `storedAt`
  (`staleAfter` стареет от серверного фетча), а `$queryCache` автоматически исключён из
  `serialize(scope)`. См. [SSR-рецепт](/ru/recipes/ssr-and-testing).
