...
Supported keys: `concurrency`, `retry`, `staleAfter`, `timeout`. Precedence:
the query's own config (inline / Store / operators / factory) > `$queryDefaults`

> built-ins; an explicit value (e.g. `timeout: 0`) always opts out of the store.
> Mutations keep their pinned `TAKE_EVERY` default.
